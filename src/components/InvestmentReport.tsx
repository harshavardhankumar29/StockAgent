...
export default function InvestmentReport({ data, onReset }: InvestmentReportProps) {
  ...
  return (
    <div className="space-y-6 py-8 max-w-4xl mx-auto animate-fade-in print-container">
      ...
      {/* Follow-up Interactive Chat [NEW] */}
      {data.id && (
        <SpotlightCard
          spotlightColor="rgba(6, 182, 212, 0.05)"
          className="bg-surface/60 border border-border rounded-xl overflow-hidden flex flex-col h-[350px] print:hidden"
        >
          ...
          >
            <input
              ...
              placeholder="e.g. Is the valuation justified given its revenue growth?"
      ...
      <div className="flex justify-center items-center gap-4 pt-4 pb-8 print:hidden">
        <button
          onClick={handlePrint}
          className="flex items-center px-5 py-2.5 font-semibold text-xs bg-white/[0.02] border border-border hover:bg-white/[0.06] text-slate-300 rounded-xl transition-all duration-300 cursor-pointer"
        >
